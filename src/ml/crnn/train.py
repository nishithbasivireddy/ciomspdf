...
from src.ml.crnn.dataset import (
    CRNNTextDataset,
    ctc_collate_fn,
)
...
DEFAULT_MANIFEST = (
    "data/ml_training/text/manifest.csv"
)
...
DEFAULT_METADATA_PATH = (
    "models/custom_ml/crnn_text_recognizer_metadata.json"
)

DEFAULT_HISTORY_PATH = (
    "models/custom_ml/crnn_training_history.json"
)

DEFAULT_TEST_REPORT_PATH = (
    "models/custom_ml/crnn_test_report.json"
)
...
def main() -> None:
    ...
    metadata = {
        "model_type": (
            "Custom CNN plus bidirectional LSTM "
            "text recognizer trained with CTC loss"
        ),
        "task": (
            "Single-line image crop transcription"
        ),
        "input_shape": [
            1,
            32,
        ...
        "model_path": args.model_path,
        "integrity_statement": (
            "The model is trained from labelled crop "
            "images. It does not consume Plain Python, "
            "OCR, LLM, or uploaded-form reference values."
        ),
        "known_limitation": (
